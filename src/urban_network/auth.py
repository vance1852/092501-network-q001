...
class Auth:
    def __init__(self,db,lock=None):
        self.db=db; self._lock=lock or threading.RLock()
        with self._lock:
    ...
    def login(self,user_id,password):
        ...
        with self._lock: self.db.execute("INSERT INTO sessions VALUES(?,?,datetime('now','+8 hours'),1)",(token,user_id)); self.db.commit()
        return token
