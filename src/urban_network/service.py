...
import functools,sqlite3,threading,uuid
from .auth import Auth
from .errors import Conflict
from .models import Reading,Segment,as_dict,canonical_instant,reading_fingerprint,utcnow
from .risk import leak_probability,score_reading
from .storage import audit,connect,rows,transaction
def _serialized(method):
    """在共享连接上串行化写事务，保证并发首写与顺序写入判定一致。"""
    @functools.wraps(method)
    def wrapper(self,*args,**kwargs):
        with self._lock:return method(self,*args,**kwargs)
    return wrapper
class NetworkService:
    def __init__(self,database=":memory:"): self.db=connect(database); self._lock=threading.RLock(); self.auth=Auth(self.db,self._lock)
    def bootstrap(self):
    ...
    def segment(self,token,segment_id):
        ...
        return dict(row)
    def _claim_slot(self,reading,instant):
        slot=self.db.execute("SELECT reading_id FROM readings WHERE segment_id=? AND sensor_id=? AND observed_at=?",(reading.segment_id,reading.sensor_id,instant)).fetchone()
        if slot:raise Conflict(f"传感器 {reading.sensor_id} 在 {instant} 的读数已存在: {slot[0]}")
    def _replay(self,row,criticality,fingerprint):
        if row["fingerprint"]!=fingerprint:raise Conflict(f"读数 {row['reading_id']} 已存在且载荷不同")
        risk=score_reading(row["pressure_kpa"],row["flow_lps"],row["acoustic_db"],criticality); alert_id="alert-"+row["fingerprint"][:18] if risk.severity in {"high","critical"} else None
        return {"reading_id":row["reading_id"],"duplicate":True,"risk":as_dict(risk),"alert_id":alert_id,"fingerprint":row["fingerprint"]}
    @_serialized
    def ingest_reading(self,token,reading):
        ...
        instant=canonical_instant(reading.observed_at); fingerprint=reading_fingerprint(reading.segment_id,reading.sensor_id,instant,reading.pressure_kpa,reading.flow_lps,reading.acoustic_db)
        try:
            with transaction(self.db):
                row=self.db.execute("SELECT * FROM readings WHERE reading_id=?",(reading.reading_id,)).fetchone()
                if row:return self._replay(row,seg[0],fingerprint)
                self._claim_slot(reading,instant)
                risk=score_reading(reading.pressure_kpa,reading.flow_lps,reading.acoustic_db,seg[0]); alert_id=None
                self.db.execute("INSERT INTO readings VALUES(?,?,?,?,?,?,?,?)",(reading.reading_id,reading.segment_id,reading.sensor_id,reading.pressure_kpa,reading.flow_lps,reading.acoustic_db,instant,fingerprint))
                if risk.severity in {"high","critical"}:
                    alert_id="alert-"+fingerprint[:18]; self.db.execute("INSERT OR IGNORE INTO alerts VALUES(?,?,?,?,?,?,?,?,?)",(alert_id,reading.segment_id,reading.reading_id,fingerprint,risk.severity,risk.score,"open",utcnow(),None))
                audit(self.db,"reading",reading.reading_id,"ingested",actor.user_id,{"risk":as_dict(risk),"alert_id":alert_id,"fingerprint":fingerprint})
